<template>
  <div class="bg-vs-card border border-vs-border rounded-xl p-5 hover:border-vs-primary/50 transition-colors">
    <!-- Header -->
    <div class="flex items-start justify-between mb-3">
      <div class="flex items-center space-x-3">
        <span class="font-mono text-sm text-vs-primary">[{{ finding.id }}]</span>
        <span
    ...
    <div class="flex items-center space-x-2 text-sm mb-3">
      <span class="text-vs-primary font-mono">{{ finding.file }}:{{ finding.line }}</span>
    </div>
    ...
    <div v-if="finding.code_snippet" v-show="expanded" class="mb-4">
      <div class="bg-vs-darker rounded-lg p-4 overflow-x-auto">
        <pre class="text-sm text-vs-text font-mono">{{ finding.code_snippet }}</pre>
      </div>
    </div>
    ...
    <!-- AI Explanation -->
    <div v-if="explanation" class="mt-4 p-4 bg-vs-darker/50 rounded-lg border border-vs-border">
